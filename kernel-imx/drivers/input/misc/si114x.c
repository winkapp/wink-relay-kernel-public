...
static int turn_on_leds(struct si114x_data *si114x) {
	...
	rc += _parm_write(si114x, CMD_PARAM_SET, PARAM_PSLED12_SELECT, 0x07);
 ...
 * of this sensor */
#ifdef CONFIG_HAS_EARLYSUSPEND	// Added by WLIU
...
static int si114x_setup(struct si114x_data *si114x) {
	...
	//t-larry
	//enhance ADC GAIN
	_parm_write(si114x, CMD_PARAM_SET, PARAM_ADC_GAIN, 0x00);

	//t-larry
	/* Set the PS sensitivity and measurement mode. */
	_parm_write(si114x, CMD_PARAM_SET, PARAM_ADC_MISC, 0x04);

	//t-larry
	//set the sensor to large IR photodiode
	_parm_write(si114x, CMD_PARAM_SET, PARAM_PS1_ADCMUX, 0x03);
	_parm_write(si114x, CMD_PARAM_SET, PARAM_PS2_ADCMUX, 0x03);
	_parm_write(si114x, CMD_PARAM_SET, PARAM_PS3_ADCMUX, 0x03);
