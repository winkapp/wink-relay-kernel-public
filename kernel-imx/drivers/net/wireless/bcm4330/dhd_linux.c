...
int dhd_ioctl_entry_local(struct net_device *net, wl_ioctl_t *ioc, int cmd)
{
    ...
    DHD_INFO(("%s pub=%p mco-mco wifi stamp  ioctl to ifdx=%d wl_ioctl=0x%x wl_ioctl->buf=0x%x wl_ioctl->len=%d 1\n", __FUNCTION__, dhd->pub, ifidx, ioc, ioc->buf, ioc->len));
	DHD_OS_WAKE_LOCK(&dhd->pub);
	ret = dhd_wl_ioctl(&dhd->pub, ifidx, ioc, ioc->buf, ioc->len);
	DHD_OS_WAKE_UNLOCK(&dhd->pub);
	DHD_INFO(("%s pub=%p mco-mco wifi stamp 2\n", __FUNCTION__,dhd->pub));
