...
dhdsdio_clkctl(dhd_bus_t *bus, uint target, bool pendok)
{
	...
	//DHD_TRACE(("%s: Enter\n", __FUNCTION__));	// wliu too many dumpings!
...
#endif /* BCMSPI */
    /// mco-mco printk("%s(%s, %s) returned:(bool)%d \n", __FUNCTION__, pfw_path, pnv_path, ret);  // mco-mco
...
dhdsdio_download_firmware(struct dhd_bus *bus, osl_t *osh, void *sdh)
{
	...
	DHD_TRACE(("%s: _dhdsdio_download_firmware...\n", __FUNCTION__));
	dhdsdio_clkctl(bus, CLK_AVAIL, FALSE);
...
dhdsdio_download_nvram(struct dhd_bus *bus)
{
	...
	{
		printk("W/ %s Warrning: nvram_file_exists does not exist \n",__FUNCTION__); // mco-mco
		return (0);
    }
	if (nvram_file_exists) {
		...
		if (image == NULL)
		{
			printk("E/ %s dhd_os_open_image(%s) failed \n",__FUNCTION__, pnv_path); // mco-mco
			goto err;
        }
	}
...
_dhdsdio_download_firmware(struct dhd_bus *bus)
{
	...
	DHD_TRACE(("%s: dhdsdio_download_nvram\n", __FUNCTION__));
	...
	DHD_TRACE(("%s: dhdsdio_download_nvram done\n", __FUNCTION__));
