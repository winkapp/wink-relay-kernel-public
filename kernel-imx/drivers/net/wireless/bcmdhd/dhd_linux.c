...
tsk_ctl_t ap_eth_ctl; /* ap netdev heper thread ctl */
#endif
...
#if 0 && defined(SUPPORT_MULTIPLE_REVISION)	// wliu + 0 to prevent *_b2_b2
	if (strlen(fw_path) != 0 &&
...
#else
    sema_init(&dhd_chipup_sem, 0); // mco-mco kernel panic in cleanup when the semaphore is released
	dhd_customer_gpio_wlan_ctrl(WLAN_POWER_ON);
#if defined(CONFIG_WIFI_CONTROL_FUNC)
	if (wl_android_wifictrl_func_add() < 0)
		goto fail_1;
