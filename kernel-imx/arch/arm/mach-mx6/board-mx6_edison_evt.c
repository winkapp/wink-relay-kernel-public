...
#include "board-mx6q_edison_evt.h"
#include "board-mx6dl_edison_evt.h"
#include <mach/imx_rfkill.h>

#define SABRESD_BT_HOST_WAKE	IMX_GPIO_NR(1, 31)
#define SABRESD_BT_WAKE		IMX_GPIO_NR(1, 22)
#define SABRESD_BT_PDN_B	IMX_GPIO_NR(1, 23)
#define SABRESD_ZB_BOOT_CONTROL IMX_GPIO_NR(4, 14)
#define SABRESD_ZB_RST		IMX_GPIO_NR(1, 4)
#define SABRESD_WL_WAKE		IMX_GPIO_NR(1, 2)
#define SABRESD_WLAN_WAKE	IMX_GPIO_NR(1, 9)
#define SABRESD_WLAN_PDN_B	IMX_GPIO_NR(1, 25)
#define SABRESD_SWITCH_1	IMX_GPIO_NR(1, 7)
#define SABRESD_SWITCH_2	IMX_GPIO_NR(1, 8)
...
#define SABRESD_LCD_RESET	IMX_GPIO_NR(1, 26)
#define SABRESD_LCD_BL		IMX_GPIO_NR(1, 30)
...
#define SABRESD_RELAY_1         IMX_GPIO_NR(7, 11)
#define SABRESD_RELAY_2         IMX_GPIO_NR(7, 12)
...
/* TOUCH */
// reset pin, irq is handled in himax driver
#define SABRESD_HX8526_CTP_RST	IMX_GPIO_NR(1, 5)
#define SABRESD_HX8526_CTP_INT	IMX_GPIO_NR(4, 11)
...
static const struct imxuart_platform_data zb_uart2_data __initconst = {
	.flags      = IMXUART_HAVE_RTSCTS,
...
{
	imx6q_add_imx_uart(2, &mx6q_sd_uart2_data);
	imx6q_add_imx_uart(1, &zb_uart2_data);
...
static struct spi_board_info imx6_sabresd_spi1_device[] __initdata = {
...
{
	spi_register_board_info(imx6_sabresd_spi1_device,
				ARRAY_SIZE(imx6_sabresd_spi1_device));
}

#ifdef CONFIG_FB_MXC_DATAIMAGE_PANEL
static struct platform_device mxcfb_dataimgae_lcd_device = {
	.name = "mxcfb_dataimage_lcd",
};
#endif
...
#ifdef CONFIG_INPUT_SI114X
static struct si114x_platform_data si114x_pdata __initdata = {
	//t-larry
	//Set the led voltage per Edison HW
	.pfd_ps_led1 = 0xA,
	.pfd_ps_led2 = 0xA,
	.pfd_ps_led3 = 0xA,
	.pfd_als_poll_interval = 500,
	.pfd_ps_poll_interval = 500,
};
#endif
...
#ifdef CONFIG_USE_IMX6Q // Commented out by WLIU
#ifdef CONFIG_TOUCHSCREEN_HIMAX_8526_D32
	{
		//IRQ, RST are request in Himax driver
		I2C_BOARD_INFO("himax-ts", 0x4a), /* Capacity Touch Screen */
	},
#endif
#endif
};
...
static struct i2c_board_info mxc_i2c2_board_info[] __initdata = {
	{
		I2C_BOARD_INFO("si70xx", 0x40), /* temperature/humidity sensor */
	},
#ifdef CONFIG_INPUT_SI114X
	{
		I2C_BOARD_INFO("si114x", 0x5a), /* Proximity sensor */
		.platform_data = &si114x_pdata,
	},
#endif
};

static struct i2c_board_info mxc_i2c3_board_info[] __initdata = {
#if CONFIG_USE_IMX6DL
#ifdef CONFIG_TOUCHSCREEN_HIMAX_8526_D32
	{
		//IRQ, RST are request in Himax driver
		I2C_BOARD_INFO("himax-ts", 0x4a), /* Capacity Touch Screen */
...
//	gpio_request(SABRESD_BT_PDN_B, "bt-reset");
//	gpio_direction_output(SABRESD_BT_PDN_B, 0);
	gpio_set_value(SABRESD_BT_PDN_B, 0);
	/* pull down reset pin at least >5ms */
	mdelay(6);
	/* pull up after power supply BT */
//	gpio_direction_output(SABRESD_BT_PDN_B, 1);
//	gpio_free(SABRESD_BT_PDN_B);
	gpio_set_value(SABRESD_BT_PDN_B, 1);
	msleep(100);
}
...
static struct gpio_keys_button edison_switchs[] = {
	GPIO_BUTTON(SABRESD_SWITCH_1, EV_SW, 1, "light-switch-1", 0, 1),
	GPIO_BUTTON(SABRESD_SWITCH_2, EV_SW, 1, "light-switch-2", 0, 1),
};

static struct gpio_keys_platform_data edison_switch_data = {
	.buttons	= edison_switchs,
	.nbuttons	= ARRAY_SIZE(edison_switchs),
};
...
static void __init imx6q_add_device_buttons(void)
{
	...
	ret = gpio_request(SABRESD_SWITCH_1, "Light Switch 1");
	if(ret) {
	  printk(KERN_ERR"Request GPIO %d Error!!\n", SABRESD_SWITCH_1);
	} else {
	  gpio_direction_input(SABRESD_SWITCH_1);
	  msleep(1);
	  gpio_set_value(SABRESD_SWITCH_1, 1);
	  gpio_export(SABRESD_SWITCH_1, false);
	}

	ret = gpio_request(SABRESD_SWITCH_2, "Light Switch 2");
	if(ret) {
	  printk(KERN_ERR"Request GPIO %d Error!!\n", SABRESD_SWITCH_2);
	} else {
	  gpio_direction_input(SABRESD_SWITCH_2);
	  msleep(1);
	  gpio_set_value(SABRESD_SWITCH_2, 1);
	  gpio_export(SABRESD_SWITCH_2, false);
	}
}

static void __init imx6q_add_relays(void)
{
	int ret;

	ret = gpio_request(SABRESD_RELAY_1, "Relay 1");
	if(ret) {
	  printk(KERN_ERR"Request GPIO %d Error!!\n", SABRESD_RELAY_1);
	} else {
	  gpio_direction_output(SABRESD_RELAY_1, 0);
	  msleep(1);
	  gpio_export(SABRESD_RELAY_1, false);
	}

	ret = gpio_request(SABRESD_RELAY_2, "Relay 2");
	if(ret) {
	  printk(KERN_ERR"Request GPIO %d Error!!\n", SABRESD_RELAY_2);
	} else {
	  gpio_direction_output(SABRESD_RELAY_2, 0);
	  msleep(1);
	  gpio_export(SABRESD_RELAY_2, false);
	}
}
...
int __init brcm_wlan_init(void);	// wliu
...
static void __init mx6_sabresd_board_init(void)
{
	...
	/* WiFi GPIO */
	ret = gpio_request(SABRESD_WLAN_PDN_B, "WiFi Power on/off");
	if (ret) {
		printk(KERN_ERR"request WLAN_PDN_B error!!\n");
		return;
	} else {
		  gpio_direction_output(SABRESD_WLAN_PDN_B, 1);
        	  msleep(1);
	          gpio_export(SABRESD_WLAN_PDN_B, false);
	}

	ret = gpio_request(SABRESD_WL_WAKE, "WL wake");
	if (ret) {
		printk(KERN_ERR"request SABRESD_WL_WAKE error!!\n");
		return;
	} else {
		  gpio_direction_output(SABRESD_WL_WAKE, 1);
        	  msleep(1);
	          gpio_export(SABRESD_WL_WAKE, false);
	}

	ret = gpio_request(SABRESD_WLAN_WAKE, "WLAN wake");
	if (ret) {
		printk(KERN_ERR"request SABRESD_WLAN_WAKE error!!\n");
		return;
	} else {
		  gpio_direction_input(SABRESD_WLAN_WAKE);
        	  msleep(1);
	          gpio_export(SABRESD_WLAN_WAKE, false);
	}

	/* ZB GPIO */
	ret = gpio_request(SABRESD_ZB_RST, "ZB Power on/off");
	if (ret) {
		printk(KERN_ERR"request SABRESD_ZB_RST error!!\n");
		return;
	} else {
		  gpio_direction_output(SABRESD_ZB_RST, 1);
        	  msleep(1);
	          gpio_export(SABRESD_ZB_RST, false);
	}

	ret = gpio_request(SABRESD_ZB_BOOT_CONTROL, "ZB PA5");
	if (ret) {
		printk(KERN_ERR"request SABRESD_ZB_RST error!!\n");
		return;
	} else {
		  gpio_direction_output(SABRESD_ZB_BOOT_CONTROL, 1);
        	  msleep(1);
	          gpio_export(SABRESD_ZB_BOOT_CONTROL, false);
	}

	/* BT GPIO */
	ret = gpio_request(SABRESD_BT_PDN_B, "BT Power on/off");
	if (ret) {
		printk(KERN_ERR"request WLAN_PDN_B error!!\n");
		return;
	} else {
		  gpio_direction_output(SABRESD_BT_PDN_B, 1);
        	  msleep(1);
	          gpio_export(SABRESD_BT_PDN_B, false);
	}

	ret = gpio_request(SABRESD_BT_WAKE, "BT host wake");
	if (ret) {
		printk(KERN_ERR"request SABRESD_BT_HOST_WAKE error!!\n");
		return;
	} else {
		  gpio_direction_output(SABRESD_BT_WAKE, 1);
        	  msleep(1);
	          gpio_export(SABRESD_BT_WAKE, false);
	}

	ret = gpio_request(SABRESD_BT_HOST_WAKE, "BT wake");
	if (ret) {
		printk(KERN_ERR"request WLAN_PDN_B error!!\n");
		return;
	} else {
		  gpio_direction_input(SABRESD_BT_HOST_WAKE);
        	  msleep(1);
	          gpio_export(SABRESD_BT_HOST_WAKE, false);
	}
	...
	// t-larry
	// Reset LCD
        ret = gpio_request(SABRESD_LCD_RESET, "LCD Reset");
        if(ret) {
          printk(KERN_ERR"Request GPIO %d Error!!\n", SABRESD_LCD_RESET);
        } else {
          gpio_direction_output(SABRESD_LCD_RESET, 1);
          gpio_export(SABRESD_LCD_RESET, false);
          msleep(5);
	  gpio_set_value(SABRESD_LCD_RESET, 0);
          msleep(5);
	  gpio_set_value(SABRESD_LCD_RESET, 1);
          msleep(5);
        }
	platform_device_register(&mxcfb_dataimgae_lcd_device);
#endif
	...
	/* SPI */
	imx6q_add_ecspi(0, &mx6q_sabresd_spi1_data);
	spi_device_init();
	...
	*/
	...
	//LCD BL
        ret = gpio_request(SABRESD_LCD_BL, "LCD backlight");
        if(ret) {
          printk(KERN_ERR"Request GPIO %d Error!!\n", SABRESD_LCD_BL);
        } else {
          gpio_direction_output(SABRESD_LCD_BL, 0);
          msleep(1);
          gpio_export(SABRESD_LCD_BL, false);
        }
...
// mco-mco  looks like the broadcom requires private vram memory
